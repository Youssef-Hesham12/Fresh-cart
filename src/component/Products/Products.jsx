...
export default function Products() {
  ...
  async function addWishList(id) {
    let response = await myWishList(id)
      .then((response) => {
        ...
        toast.success(' it has been Successfully Added!')
        // btn.current.classList.replace('text-[#212529]', 'text-[red]')
        console.log(response);
  ...
  return (
    ...
    <div className="mx-auto container max-w-2xl sm:px-6 sm:pt-12 lg:max-w-7xl lg:px-8">
      ...
      <div className="my-3">
        <input type="search" id="search" onChange={(e) => setSearchItem(e.target.value)} name="search" className="bg-white border focus:ring-[#4FA74F] focus:border-[#4FA74F] border-gray-300 text-gray-900 text-sm rounded-md block w-3/4 p-2.5 my-12 mx-auto dark:bg-gray-700 dark:border-gray-600  dark:text-white  placeholder:text-gray-600 " placeholder='search....' />
      </div>
          ...
          <div className='container mx-auto' key={product.id}>
            <div className="products rounded-md py-6 px-2 transition-all duration-500 card group">
              <Link to={`/productdetails/${product.id}/${product.category.name}`}>
                ...
                <h6 className='mt-0 mb-3 sm:ms-2 ms-5 font-medium'>{product.title.split(' ').slice(0, 2).join(' ')}</h6>
              ...
              <div className="flex justify-between items-center">
                <button onClick={() => addProduct(product.id)} type="button" className=" group-hover:translate-y-0 group-hover:opacity-100 transition-all  translate-y-80 duration-500 opacity-0 mt-6 py-[6px] px-7  text-white bg-[#3FA74F] hover:bg-green-700 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm text-center inline-flex items-center ">
                  + Add
                </button>
